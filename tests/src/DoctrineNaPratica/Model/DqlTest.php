<?php
//tests/src/DoctrineNaPratica/Model/QdlTest.php
...
Código apresentado anteriormente no livro - página 61
...
//buscar lições que tenham 100 de progresso
$query = $this->em->createQuery('SELECT p FROM DoctrineNapratica\Model\Progress p\
JOIN p.lesson l JOIN l.course c WHERE p.percent = 100');
$result = $query->getResult();

//buscar usuários usando parâmetros
$query = $this->em->createQuery('SELECT u FROM DoctrineNapratica\Model\User u WHE\
RE u.login = :login');
$query->setParameter('login', 'Bob');
$users = $query->getResult();

//buscar usuários usando múltiplos parâmetros
$query = $this->em->createQuery('SELECT u FROM DoctrineNapratica\Model\User u WHE\
RE u.login = :login or u.email = :email');
$query->setParameters(
array('login' => 'userA', 'email' => 'userA@domain.com')
);
$users = $query->getResult();
